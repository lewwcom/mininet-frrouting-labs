...
Vagrant.configure("2") do |config|
  ...
  config.vm.base_mac = "00:50:56:c0:ef:c0" # 00:50:56 is OUI of VMWare VM
  config.vm.base_address = "192.168.137.10"
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    echo 'mpls-router
mpls-iptunnel' > /etc/modules-load.d/mpls.conf
...
https://deb.frrouting.org/frr $(lsb_release -s -c) $FRRVER" |
    ...
    DEBIAN_FRONTEND=noninteractive apt install -y \
      ...
      iperf iperf3 wireshark xterm xauth \
      mininet bridge-utils openvswitch-switch \
      frr frr-pythontools
